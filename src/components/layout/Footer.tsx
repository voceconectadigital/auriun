...
export function Footer() {
  ...
  return (
    <footer className="bg-brand-navy text-white">
      ...
      {/* CTA band — brighter blue, clear separation from deep navy footer */}
        ...
        <Container className="px-5 pt-14 sm:px-8 md:pt-16 lg:px-10 lg:pt-20">
          <div className="footer-cta-band grid overflow-hidden bg-[#1a7bc9] shadow-[0_18px_48px_rgba(3,18,32,0.28)] lg:grid-cols-[1.25fr_auto]">
            <div className="px-7 py-9 sm:px-10 sm:py-11 lg:px-12 lg:py-12">
              ...
              <h2 className="mt-3 max-w-xl text-[clamp(1.5rem,2.4vw,2.15rem)] font-bold leading-tight tracking-[-0.02em] text-white">
                Fale com a equipe técnica e comercial da Auriun.
              </h2>
              <p className="mt-4 max-w-lg text-sm leading-relaxed text-white/80 sm:text-[0.975rem]">
                Orçamentos, especificação e sourcing para demandas industriais de alta exigência.
              </p>
            </div>
            <div className="flex w-full items-center justify-center border-t border-white/20 px-7 py-7 sm:px-10 lg:w-auto lg:border-l lg:border-t-0 lg:px-12">
              <Link
      ...
      <div className="footer-main border-t border-white/10">
        <Container className="section-pad grid gap-14 pt-16 pb-16 md:grid-cols-2 md:pt-20 lg:grid-cols-12 lg:gap-12 lg:pb-20 lg:pt-20">
          <div className="lg:col-span-4">
            <div className="inline-flex bg-white px-4 py-3">
              <Logo className="h-14 w-auto max-w-[150px]" />
            </div>
            <p className="mt-7 max-w-sm text-[0.975rem] leading-relaxed text-white/65">
          ...
          <div className="lg:col-span-2">
            ...
            <ul className="mt-5 space-y-3 text-[0.9375rem] text-white/60">
              ...
              <li>
                <Link to="/a-auriun/" className="transition-colors hover:text-white">
                  A Auriun
                </Link>
              </li>
              <li>
                <Link to="/solucoes/" className="transition-colors hover:text-white">
                  Soluções
                </Link>
              </li>
              <li>
                <Link to="/segmentos/" className="transition-colors hover:text-white">
                  Segmentos
                </Link>
              </li>
              <li>
          ...
          <div className="lg:col-span-3">
            ...
            <ul className="mt-5 space-y-3 text-[0.9375rem] text-white/60">
              {products.slice(0, 4).map((item) => (
                <li key={item.slug}>
                  <Link to={productPath(item.slug)} className="transition-colors hover:text-white">
                    {item.shortTitle}
                  </Link>
                </li>
              ))}
              {services.slice(0, 2).map((item) => (
                <li key={item.slug}>
                  <Link to={servicePath(item.slug)} className="transition-colors hover:text-white">
                    {item.shortTitle}
                  </Link>
                </li>
              ))}
              <li>
                <Link
                  to="/solucoes/"
                  className="inline-flex items-center gap-1.5 font-medium text-brand-orange transition-colors hover:text-white"
                >
                  Ver todas
          ...
          <div className="lg:col-span-3">
            ...
            <ul className="mt-5 space-y-3 text-[0.9375rem] text-white/60">
              {phone && hasValue(CONTACT.phone) ? (
                <li>
                  <a href={phone} className="inline-flex items-center gap-2 transition-colors hover:text-white">
              ...
              {hasValue(CONTACT.address) ? (
                ...
                <li className="text-white/40">Dados de contato em atualização.</li>
              )}
            ...
            <div className="mt-8">
              ...
              <ul className="mt-4 flex flex-wrap gap-x-3 gap-y-2 text-xs text-white/50">
                {segments.map((item) => (
                  <li key={item.slug}>
                    <Link to={segmentPath(item.slug)} className="transition-colors hover:text-white">
                      {item.shortTitle}
                    </Link>
                  </li>
                ))}
      ...
      <div className="border-t border-white/10">
        <Container className="flex flex-col gap-3 py-7 sm:flex-row sm:items-end sm:justify-between">
          <p
            className="select-none text-[clamp(2.5rem,8vw,5.5rem)] font-bold leading-none tracking-[-0.06em] text-white/[0.06]"
            aria-hidden
          >
            AURIUN
          </p>
